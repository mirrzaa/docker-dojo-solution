...
router.get('/', function(req, res, next) {
  ...
  async.parallel([
      function(callback){
        ...
        rest.get('http://search:8100/api/artists/search?artist=' + artist).on('complete', function(data) {
          console.log('Artist ID is ' + data["id"]);
          rest.get('http://charts:8100/api/charts/' + data["id"]).on('complete', function(data) {
      ...
      function(callback){
          ...
          rest.get('http://search:8100/api/tracks/search?title=' + title + '&artist=' + artist).on('complete', function(data) {
            console.log('Title ID is ' + data["id"]);

            rest.get('http://images:8100/api/covers/' + data["id"]).on('complete', function(data) {
